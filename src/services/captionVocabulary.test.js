...
test('joins parts comma-separated with normalized whitespace, terminated like a sentence', () => {
  assert.equal(
    buildVocabularyHint(['CDX Studio', '  Seedance   shots ', 'ComfyUI']),
    'CDX Studio, Seedance shots, ComfyUI.'
  )
})

test('drops empties, non-strings, and case-insensitive duplicates', () => {
  assert.equal(
    buildVocabularyHint(['CDX Studio', '', null, undefined, 'velorn', '  ', 'VELORN', 'Qwen']),
    'CDX Studio, Qwen.'
  )
})

test('ignores default app-planted strings', () => {
  assert.equal(buildVocabularyHint(['Sample Text', 'CDX Studio']), 'CDX Studio.')

...
export function buildFcpXml({
  ...
  const resourceEntries = [
    `    <format id="${formatId}" name="CDX Studio ${width}x${height} ${timebase.fps}fps" frameDuration="${timebase.frameDuration}" width="${width}" height="${height}" colorSpace="1-1-1 (Rec. 709)"/>`,
  ...
  return [
    ...
    '  </resources>',
    '  <library>',
    `    <event name="${escapeXml(sanitizeName(projectName, 'CDX Studio Project'))}">`,
    ...
    '          <spine>',
    `            <gap name="CDX Studio Timeline" offset="0s" start="0s" duration="${formatFrames(sequenceDurationFrames, timebase)}">`,
    ...clipElements,
    '            </gap>',
    '          </spine>',
    '        </sequence>',

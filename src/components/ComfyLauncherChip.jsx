            ...
            {state.state === 'external' && (
              <div className="flex items-start gap-2 rounded-md bg-sky-500/10 border border-sky-500/30 px-2.5 py-2 text-[11px] text-sky-200">
                ...
                <div className="flex-1">
                  <div>
                    ComfyUI is already running but CDX Studio can't identify its process, so Stop and Restart are disabled. This usually means netstat/lsof isn't available. Try "Take control" to retry, or stop it from the window you started it from.
                  </div>
                ...
                <div className="flex items-start gap-2">
                  ...
                  <div className="flex-1">
                    ...
                    <div className="mt-0.5 text-amber-100/90 leading-snug">
                      {portOwner?.pid
                        ? <>Held by <span className="font-mono">{portOwner.name || 'pid'} (pid {portOwner.pid})</span>. {portOwner.name && /python|comfy/i.test(portOwner.name) ? 'That looks like ComfyUI already running.' : 'CDX Studio will not be able to start ComfyUI until that process releases the port.'}</>
                        : 'Another process is bound to this port. CDX Studio will not be able to start ComfyUI until that process releases the port.'}
                    </div>
          ...
          {isMacPlatform && launcherMode === 'mac-app' && (
            <div className="px-3.5 py-3 space-y-2 border-b border-sf-dark-700">
              ...
              <div className="text-[11px] text-sf-text-primary truncate" title={config.macAppPath}>
                {config.macAppPath || (
                  <span className="italic text-sf-text-muted">No app configured. Pick ComfyUI.app to open it from CDX Studio.</span>
                )}
              </div>
              <div className="text-[10px] text-sf-text-muted">CDX Studio opens the Mac app and waits for the configured endpoint.</div>
            </div>
          )}
          ...
          <div className="px-3.5 py-3 space-y-2 border-b border-sf-dark-700">
            ...
            <div className="text-[11px] text-sf-text-primary truncate" title={config.launcherScript}>
              {config.launcherScript || (
                <span className="italic text-sf-text-muted">No launcher configured. Pick your run_nvidia_gpu.bat to let CDX Studio start ComfyUI for you.</span>
              )}
            </div>

...
function buildExternalLlmPrompt(data, currentScript) {
  return [
    'Write a CDX Studio Director Mode script for a small-business ad using this exact structure.',
    '',
    ...
    '- Do not ask CDX Studio to render text into images. Reserve space for editor-native text instead.',
      ...
      <div className="mt-3 rounded-lg border border-sf-dark-700 bg-sf-dark-900/70 p-3">
        ...
        <div className="mt-3 border-t border-sf-dark-700 pt-3">
          <div className="flex flex-col gap-3 md:flex-row md:items-start md:justify-between">
            <div className="min-w-0">
              <div className="flex flex-wrap items-center gap-2">
                <span className="text-[10px] font-semibold uppercase tracking-wider text-sf-text-muted">CDX Studio bridge</span>
                <span className={`rounded-full border px-2 py-0.5 text-[10px] ${bridgeBadge.className}`}>
                  {bridgeBadge.label}
                </span>
              </div>
              <p className="mt-1 text-[10px] leading-4 text-sf-text-muted">
                Adds a Send to CDX Studio button inside ComfyUI. Import JSON stays available as the fallback.
              </p>
              {bridgeMessage && (
            ...
            <div className="grid w-full shrink-0 gap-2 sm:w-auto sm:min-w-[160px]">
              <button
                ...
                className="inline-flex items-center justify-center gap-1.5 rounded border border-sf-accent/50 bg-sf-accent/10 px-2 py-1.5 text-[10px] font-semibold text-sf-accent transition-colors hover:bg-sf-accent/20 disabled:cursor-not-allowed disabled:border-sf-dark-600 disabled:bg-sf-dark-800 disabled:text-sf-text-muted"
                title={bridgeState === 'unavailable' ? 'Choose a ComfyUI folder or configure the launcher first.' : 'Install the bundled CDX Studio Bridge into ComfyUI custom_nodes.'}
              >
    ...
    <div className="space-y-4">
      ...
      {step === 'setup' && (
        <div className="rounded-xl border border-sf-dark-700 bg-sf-dark-900/60 p-4 space-y-4">
          <div>
            <div className="text-[10px] uppercase tracking-[0.14em] text-sf-accent">CDX Studio asks</div>
            ...
            <div className="flex flex-wrap items-start justify-between gap-3">
              <div>
                ...
                <p className="mt-1 max-w-3xl text-[11px] leading-relaxed text-sf-text-muted">
                  No CDX Studio API key or setup required. Copy this prompt into ChatGPT, Claude, Gemini, or another LLM, then paste the result back into the editable Director Script below.
                </p>
              </div>

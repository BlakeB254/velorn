    ...
    <div className="space-y-5">
      ...
      <div className="rounded-lg border border-sf-dark-700 bg-sf-dark-900/70 px-4 py-3.5">
        ...
        {state.state === 'external' && (
          <div className="mt-2 flex items-start gap-2 rounded bg-sky-500/10 border border-sky-500/30 px-2 py-1.5 text-[11px] text-sky-200">
            ...
            <div>
              ComfyUI is already running, but CDX Studio didn't start it. Stop it from the window where you launched it and hit Start to let CDX Studio manage it (you'll get auto-restarts after node-pack installs).
            </div>
          </div>
        )}
      </div>
      ...
      {isMacPlatform && launcherMode === 'mac-app' && (
        <div>
          ...
          <div className="bg-sf-dark-800 border border-sf-dark-600 rounded px-3 py-2 text-xs text-sf-text-primary truncate min-h-[34px]">
            {config.macAppPath || (
              <span className="italic text-sf-text-muted">No app configured. Pick ComfyUI.app to let CDX Studio open it for you.</span>
            )}
          </div>
          <p className="text-[11px] text-sf-text-muted mt-1.5">
            CDX Studio opens the Mac app, waits for the configured ComfyUI endpoint, and can ask macOS to quit or reopen it.
          </p>
      ...
      <div>
        ...
        <div className="bg-sf-dark-800 border border-sf-dark-600 rounded px-3 py-2 text-xs text-sf-text-primary truncate min-h-[34px]">
          {config.launcherScript || (
            <span className="italic text-sf-text-muted">No launcher configured. Pick your run_nvidia_gpu.bat (or equivalent) to let CDX Studio start ComfyUI for you.</span>
          )}
        </div>
        {candidates.length > 0 && (
      ...
      <div className="rounded-lg border border-sf-dark-700 bg-sf-dark-900/60 px-4 py-3 space-y-3">
        ...
        <div className="flex items-start justify-between gap-3">
          <div className="min-w-0">
            <div className="text-sm text-sf-text-primary">Auto-start ComfyUI when CDX Studio launches</div>
            <p className="text-[11px] text-sf-text-muted mt-0.5">Off by default. When on, CDX Studio starts ComfyUI automatically as soon as the app opens.</p>
          </div>
          <Toggle
        ...
        <div className="flex items-start justify-between gap-3">
          <div className="min-w-0">
            <div className="text-sm text-sf-text-primary">Keep ComfyUI running after CDX Studio quits</div>
            <p className="text-[11px] text-sf-text-muted mt-0.5">When on, closing CDX Studio leaves ComfyUI open in the background without asking.</p>
          </div>
          <Toggle
        ...
        <div className="flex items-start justify-between gap-3">
          <div className="min-w-0">
            ...
            <p className="text-[11px] text-sf-text-muted mt-0.5">On by default. CDX Studio passes <code className="px-1 rounded bg-sf-dark-800">--disable-auto-launch</code> so the classic ComfyUI tab doesn't steal focus each time it boots.</p>
          </div>
          <Toggle
      ...
      <div className="rounded-lg border border-sf-dark-700 bg-sf-dark-900/60 px-4 py-3 space-y-3">
        ...
        <div>
          ...
          />
          <p className="text-[11px] text-sf-text-muted mt-1">If ComfyUI hasn't responded on /system_stats within this many seconds after launch, CDX Studio will give up and stop the process.</p>
        </div>
        <div>
      ...
      <div className="flex items-start gap-2 text-[11px] text-sf-text-muted">
        ...
        <div>
          CDX Studio talks to ComfyUI on <code className="px-1 rounded bg-sf-dark-800">{state.httpBase || 'http://127.0.0.1:8188'}</code>. Change the port in

# CDX Studio app plugin (`cdx-studio-app`)
...
| | `cdx-studio-app` (this one) | `cdx-studio` |
|---|---|---|
| Ships with | the app repo | the CDX platform config repo |
| MCP | HTTP → the running app on `:19790` | stdio → kimodo, Blender, reconstruction routing |
| Owns | the app surface: project, timeline, storyboard, cuts, export | the creative-context doctrine: the ten layers, references, prompt assembly, spatial |
| Needs | the desktop app open | nothing (degrades when the app is closed) |

They used to share the name `cdx-studio`, which meant installing this one on a CDX platform box
collided with the internal plugin. This one is now `cdx-studio-app`. **On a CDX box, install the
internal `cdx-studio` plugin** — it bridges to this same MCP server via `studio_app_call` and adds
the context-layer tooling. Install this one when you are running CDX Studio standalone.

Where both carry the same doctrine (the context stack, reference cards), the internal plugin is
canonical and is kept current first.

## Install

```
/plugin marketplace add /path/to/cdx-studio
/plugin install cdx-studio-app
```
...
`cdx-studio-app` → `http://127.0.0.1:19790/mcp`

Started automatically whenever the CDX Studio app is open, exposing ~182 tools across the project,
...
| Skill | Covers |
|---|---|
| `cdx-studio-mcp` | The tool surface, how to find the right tool among ~182, and the preview-before-apply discipline. |

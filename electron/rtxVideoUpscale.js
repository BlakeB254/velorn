...
function getRuntimeCandidates({ userDataPath, comfyRootPath = '', explicitPythonPath = '' }) {
  ...
  const candidates = [
    { pythonPath: managed.pythonPath, kind: 'managed', label: 'CDX Studio RTX runtime' },
  ]
...
async function checkRtxRuntime(options = {}) {
  ...
  return {
    ...
    error: failures.length > 0
      ...
      : 'Install the optional CDX Studio RTX runtime to enable direct 4K upscaling. ComfyUI is not required.',
  }
}
...
function downloadFile(urlText, outputPath, options = {}, redirectsRemaining = 5) {
  ...
  return new Promise((resolve, reject) => {
    ...
    signal?.addEventListener?.('abort', abort, { once: true })
    request = transport.get(url, { headers: { 'User-Agent': 'CDX Studio RTX Runtime Installer' } }, (response) => {
...
async function runRtxVideoUpscale(options = {}) {
  ...
  if (!helperPath || !fs.existsSync(helperPath)) throw new Error('The CDX Studio RTX helper is missing.')
  if (!ffmpegPath || !fs.existsSync(ffmpegPath) || !ffprobePath || !fs.existsSync(ffprobePath)) {
    throw new Error('CDX Studio could not find its FFmpeg tools.')
  }

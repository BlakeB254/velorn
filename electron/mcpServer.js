...
function getClipVisualSource(snapshot, clip, asset) {
  ...
  if (posterPath) {
    return {
      ...
      description: 'Representative poster frame generated by Velorn.',
    }
  }
...
function getEffectClipForMcp(snapshot, clipId) {
  ...
  if (!id) return { error: 'Provide clipId from get_timeline, or select exactly one visual clip in Velorn.' }
...
function resolveSolidColorPlan(snapshot, args = {}) {
  ...
  if (!timeline || !project) return { error: 'Open a saved Velorn project and timeline before creating a solid color.' }
...
function resolveCreateTimelinePlan(snapshot, args = {}) {
  const project = snapshot?.project || null
  if (!project) return { error: 'Open a saved Velorn project before creating a sequence.' }
...
function resolveCreateAssetFolderPlan(snapshot, args = {}) {
  const project = snapshot?.project || null
  if (!project) return { error: 'Open a saved Velorn project before creating an asset folder.' }
...
function resolveMoveAssetsToFolderPlan(snapshot, args = {}) {
  const project = snapshot?.project || null
  if (!project) return { error: 'Open a saved Velorn project before moving assets.' }
...
function buildAiReviewPasses(snapshot) {
  ...
  return {
    purpose: 'Use these recipes as safe AI review passes for the open Velorn project. Prefer previewOnly before write actions that alter clips or markers.',
    timeline: timeline ? {
      id: timeline.id,
      name: timeline.name,
    ...
    recipes: [
      ...
      },
      {
        ...
        goal: 'Create or duplicate a Velorn project before risky AI edits or fresh automated layouts.',
      ...
      },
      {
        ...
        goal: 'Add, update, or remove Velorn native transitions such as dissolves, fades, wipes, slides, zooms, blur, and dip-to-black style edits.',
      ...
      },
      {
        ...
        goal: 'Turn a written creative brief into source images or videos that can be assembled into a new Velorn sequence.',
...
function resolveExportDeliveryPlan(snapshot, args = {}) {
  ...
  const project = snapshot?.project || null
  if (!project || !timeline) {
    return { error: 'Open a saved Velorn project and timeline before exporting.' }
  }
  ...
  const safeProjectName = sanitizeExportBaseName(project.name || 'Velorn')
  ...
  if (!timeline) {
    return {
      ...
      findings: [{
        ...
        message: 'Velorn does not currently have an active timeline snapshot.',
      }],
    }
  }
...
function createToolDefinitions() {
  return [
    {
      name: 'get_project',
      description: 'Return a concise summary of the open Velorn project, current timeline, asset counts, and MCP snapshot freshness.',
      inputSchema: {
        type: 'object',
        properties: {},
      },
    ...
    },
    {
      ...
      description: 'Return practical MCP agent recipes and recommended workflows for Velorn. This is a clearer alias for get_ai_review_passes and should be used when a user asks what the MCP can do.',
      inputSchema: {
        type: 'object',
        properties: {},
      },
    ...
    {
      name: 'studio_flow',
      description: 'Return the stage rail: script → cast → scenes → storyboard → flf → video → review → edit → deliver, with stage_reached and stage_blocked_at. Includes take-chain readiness and the VSE audio plan.',
      inputSchema: { type: 'object', properties: {} },
    },
    {
    ...
    },
    {
      ...
      description: 'Diagnose the local ComfyUI connection used by Velorn. Checks the configured localhost port, ComfyUI API endpoints, launcher state, port owner, likely install mode such as portable/Desktop/Docker/manual, and returns support-friendly next steps.',
      inputSchema: {
        type: 'object',
        properties: {
          port: { type: 'integer', description: 'Optional local ComfyUI port to test. Defaults to Velorn Settings > ComfyUI Connection.' },
    ...
    },
    {
      ...
      description: 'Set the local ComfyUI connection port used by Velorn. Supports previewOnly so assistants can propose the change before applying it. This changes Velorn settings only; it does not restart ComfyUI or edit launcher scripts.',
      inputSchema: {
        type: 'object',
        properties: {
          port: { type: 'integer', description: 'Local ComfyUI port to save, for example 8188.' },
          previewOnly: { type: 'boolean', description: 'When true, returns the proposed before/after setting without changing Velorn.' },
        },
        required: ['port'],
      },
    },
      ...
      inputSchema: {
        ...
        properties: {
          strategy: {
            ...
            description: 'Repair strategy. useReachablePort finds a reachable local ComfyUI port and points Velorn at it.',
          },
          candidatePorts: {
            type: 'array',
            items: { type: 'integer' },
          ...
          previewOnly: { type: 'boolean', description: 'When true, returns the proposed repair without changing Velorn. Defaults to true.' },
        },
      },
    },
    {
      name: 'guide_comfyui_setup',
      description: 'Beginner-friendly Velorn-to-ComfyUI setup wizard. Diagnoses the current connection, probes common local ComfyUI ports, explains what to do for Portable/Desktop/Docker/manual installs, and can preview or apply the safe port-setting fix when ComfyUI is found on a different port.',
      inputSchema: {
        type: 'object',
        properties: {
          installType: {
          ...
          applyFix: {
            type: 'boolean',
            description: 'When true and previewOnly=false, update Velorn to the reachable ComfyUI port if a safe port mismatch is found. Defaults to false.',
          },
          previewOnly: {
            type: 'boolean',
            description: 'When true, diagnose and propose the setup/fix without changing Velorn. Defaults to true.',
          },
          nodeClasses: {
            type: 'array',
            items: { type: 'string' },
    ...
    },
    {
      ...
      description: 'Preview or apply ComfyUI launcher actions through Velorn: start, stop, or restart. Defaults to previewOnly for safety. Stop/restart can interrupt running generations and only work for ComfyUI processes owned by Velorn.',
      inputSchema: {
        type: 'object',
        properties: {
          action: {
    ...
    },
    {
      ...
      description: 'Return recent ComfyUI launcher log lines from Velorn, with a lightweight summary of common support issues like port conflicts, import errors, missing models/files, and CUDA memory errors.',
      inputSchema: {
        type: 'object',
        properties: {
    ...
    },
    {
      ...
      inputSchema: {
        ...
        properties: {
          ...
          port: {
            type: 'integer',
            description: 'Optional local ComfyUI port to test. Defaults to Velorn Settings > ComfyUI Connection.',
          },
          timeoutMs: {
            type: 'integer',
    ...
    },
    {
      ...
      inputSchema: {
        ...
        properties: {
          timeSeconds: {
            type: 'number',
            description: 'Optional timeline time in seconds. Defaults to the current Velorn playhead.',
          },
          frame: {
            type: 'integer',
    ...
    },
    {
      ...
      inputSchema: {
        ...
        properties: {
          ...
          workflowId: {
            type: 'string',
            description: 'Velorn workflow id to select in Generate. Defaults to ltx23-i2v.',
          },
          category: {
            type: 'string',
          ...
          prompt: {
            type: 'string',
            description: 'Prompt to prefill. If omitted, Velorn tries to reuse prompt metadata from the source clip/asset.',
          },
          negativePrompt: {
            type: 'string',
    ...
    },
    {
      ...
      description: 'Queue the generation currently staged in the Generate tab by using the same queue path as Velorn’s Queue button. Use previewOnly first. Applying can start local/credit-backed generation depending on the selected workflow, so only call with previewOnly=false after explicit user approval.',
      inputSchema: {
        type: 'object',
        properties: {
          previewOnly: {
    ...
    },
    {
      ...
      inputSchema: {
        ...
        properties: {
          ...
          prompt: {
            type: 'string',
            description: 'Prompt for every variation. If omitted, Velorn tries to reuse prompt metadata from the source clip/asset.',
          },
          negativePrompt: {
            type: 'string',
    ...
    },
    {
      ...
      inputSchema: {
        ...
        properties: {
          ...
          seed: {
            type: 'integer',
            description: 'Optional seed. If omitted, Velorn generates one.',
          },
          durationSeconds: {
            type: 'number',
          ...
          assetFieldIds: {
            type: 'object',
            description: 'Optional map of imported template assetSelect field IDs to Velorn asset IDs, for templates with required extra media inputs.',
            additionalProperties: { type: 'string' },
          },
          folderId: {
            type: 'string',
            description: 'Optional existing Velorn asset folder ID where generated result assets should be organized after import.',
          },
          outputFolderId: {
            type: 'string',
    ...
    },
    {
      ...
      inputSchema: {
        ...
        properties: {
          ...
          folderId: {
            type: 'string',
            description: 'Optional existing Velorn asset folder ID where generated result assets should be organized after import. Use create_asset_folder first when a new folder is needed.',
          },
          outputFolderId: {
            type: 'string',
            description: 'Alias for folderId.',
    ...
    },
    {
      ...
      inputSchema: {
        ...
        properties: {
          startSeconds: {
            type: 'number',
            description: 'Optional range start in seconds. Defaults to the current Velorn playhead.',
          },
          endSeconds: {
            type: 'number',
    ...
    },
    {
      ...
      description: 'List recently opened Velorn projects (name, path, last modified, whether currently open). Works with no project open. Use with open_project to recover after an app restart or switch projects.',
      inputSchema: {
        type: 'object',
        properties: {
    ...
    {
      name: 'open_project',
      description: 'Open a Velorn project by folder path or by recent-project name. Works with no project open; replaces the currently open project (work is autosaved). Defaults to previewOnly.',
      inputSchema: {
        type: 'object',
        properties: {
    ...
    },
    {
      ...
      description: 'Set or clear label colors for current timeline clips by explicit clipIds or a safe built-in filter. Non-destructive and undoable in Velorn.',
      inputSchema: {
        type: 'object',
        properties: {
          color: {
    ...
    },
    {
      ...
      description: 'Enable or disable current timeline clips by explicit clipIds or a safe built-in filter. Use previewOnly first for AI-assisted edit decisions. Undoable in Velorn.',
      inputSchema: {
        type: 'object',
        properties: {
          enabled: {
    ...
    },
    {
      ...
      description: 'Add one or more labeled timeline markers at explicit times, frames, or the current playhead. Non-destructive and undoable in Velorn.',
      inputSchema: {
        type: 'object',
        properties: {
          markers: {
    ...
    },
    {
      ...
      description: 'Remove timeline markers by ID, all markers, marker color, label text, or time range. Undoable in Velorn; use previewOnly for safety.',
      inputSchema: {
        type: 'object',
        properties: {
          all: {
    ...
    },
    {
      ...
      description: 'Rename, recolor, or move existing timeline markers by ID, color, label text, or time range. Undoable in Velorn; use previewOnly before changing many review markers.',
      inputSchema: {
        type: 'object',
        properties: {
          all: {
    ...
    {
      name: 'undo',
      description: 'Undo the latest Velorn timeline edit or sequence/project-structure edit. Scope can be auto, timeline, or project. Low-risk and mirrors the app undo stack.',
      inputSchema: {
        type: 'object',
        properties: {
    ...
    {
      name: 'redo',
      description: 'Redo the latest Velorn timeline edit or sequence/project-structure edit. Scope can be auto, timeline, or project. Low-risk and mirrors the app redo stack.',
      inputSchema: {
        type: 'object',
        properties: {
    ...
    {
      name: 'select_assets',
      description: 'Select/preview project assets by ID, name, type, folder, status, or latest match. Velorn currently previews the first matched asset.',
      inputSchema: {
        type: 'object',
        properties: {
    ...
    },
    {
      ...
      description: 'Preview or restore an in-memory MCP project checkpoint created during this Velorn session. Defaults to previewOnly because it can replace current timeline/asset state.',
      inputSchema: {
        type: 'object',
        properties: {
    ...
    {
      name: 'relink_asset',
      description: 'Preview or relink an existing Velorn asset record to a different local file path. Useful after check_media_health finds missing media. This updates Velorn project metadata only; it does not copy media. Defaults to previewOnly.',
      inputSchema: {
        type: 'object',
        properties: {
    ...
    },
    {
      ...
      description: 'Preview or run a small ordered batch of approved Velorn MCP write actions. Use this when an agent has already planned several safe steps and wants one checkpointed apply pass. Defaults to previewOnly.',
      inputSchema: {
        type: 'object',
        properties: {
          steps: {
    ...
    {
      name: 'create_project',
      description: 'Preview or create a new Velorn project in the configured Projects folder. Applying opens the new empty project. Defaults to previewOnly so agents cannot accidentally create folders without approval.',
      inputSchema: {
        type: 'object',
        properties: {
          name: {
          ...
          width: {
            type: 'integer',
            description: 'Optional project width. Defaults to Velorn new-project defaults.',
          },
          height: {
            type: 'integer',
            description: 'Optional project height. Defaults to Velorn new-project defaults.',
          },
          fps: {
            type: 'number',
    ...
    },
    {
      ...
      description: 'Preview or duplicate a Velorn project folder. Defaults to the current open project, copies the whole folder, remaps saved paths, creates a sibling copy, and opens the duplicate on apply.',
      inputSchema: {
        type: 'object',
        properties: {
          sourceProjectPath: {
    ...
    },
    {
      ...
      description: 'Preview or create a new Velorn sequence/timeline with a name, optional dimensions, fps, duration, and optional switch-to-new-sequence behavior. Defaults to previewOnly; applying is undoable in Velorn.',
      inputSchema: {
        type: 'object',
        properties: {
          name: {
          ...
          switchToTimeline: {
            type: 'boolean',
            description: 'When true, switch Velorn to the new sequence after creation. Defaults to true.',
          },
          activate: {
            type: 'boolean',
    ...
    },
    {
      ...
      description: 'Preview or create Velorn asset-panel folders. Accepts a single folder name or a nested path like "AI Spots / July 4th Demo"; reuses existing matching folders by default and creates only missing folders.',
      inputSchema: {
        type: 'object',
        properties: {
          name: {
    ...
    },
    {
      ...
      description: 'Preview or move existing Velorn assets into an asset-panel folder. Can target explicit asset IDs/names or safe filters such as rootOnly + constantsOnly. Defaults to previewOnly and can create a missing target folder path before moving.',
      inputSchema: {
        type: 'object',
        properties: {
          targetFolderId: {
    ...
    {
      name: 'add_track',
      description: 'Create a new timeline track. Video tracks are added at the top of the stack, which is useful before adding another text/title layer. Undoable in Velorn.',
      inputSchema: {
        type: 'object',
        properties: {
          type: {
    ...
    {
      name: 'remove_track',
      description: 'Preview or remove a timeline track and all clips on it. Velorn protects the last track of each type. Use previewOnly first.',
      inputSchema: {
        type: 'object',
        properties: {
          trackId: { type: 'string', description: 'Track ID from get_timeline.' },
    ...
    },
    {
      ...
      description: 'Preview or switch the active Velorn sequence/timeline. Useful when an agent creates review timelines and then needs to work inside one.',
      inputSchema: {
        type: 'object',
        properties: {
    ...
    {
      name: 'add_transition',
      description: 'Preview or add a native Velorn transition. Use clipAId+clipBId for a between-clips transition on the same track, or clipId+edge for an in/out edge transition. Undoable in Velorn.',
      inputSchema: {
        type: 'object',
        properties: {
    ...
    },
    {
      ...
      description: 'Preview or update a native transition type, duration, alignment, or settings. Undoable in Velorn.',
      inputSchema: {
        type: 'object',
        properties: {
    ...
    },
    {
      ...
      description: 'Preview or remove one or more native Velorn transitions. Undoable in Velorn.',
      inputSchema: {
        type: 'object',
        properties: {
    ...
    {
      name: 'move_clips',
      description: 'Preview or move one or more clips to a track and/or start time. Use for AI timeline layout cleanup, review lanes, and repositioning generated assets. Undoable in Velorn.',
      inputSchema: {
        type: 'object',
        properties: {
          ...
          resolveOverlaps: { type: 'boolean', description: 'When true, use Velorn overwrite/overlap resolution. Defaults to false.' },
    ...
    {
      name: 'trim_clips',
      description: 'Preview or update clip timing/trim values for one or more clips. Supports startSeconds, durationSeconds, trimStartSeconds, and trimEndSeconds. Undoable in Velorn.',
      inputSchema: {
        type: 'object',
        properties: {
    ...
    {
      name: 'delete_clips',
      description: 'Preview or delete timeline clips by ID or simple filter. Supports ripple delete. Destructive but undoable in Velorn; use previewOnly first.',
      inputSchema: {
        type: 'object',
        properties: {
    ...
    },
    {
      ...
      description: 'Preview or place a project asset on the active timeline. Can target a specific asset id/name or the latest generated asset, choose playhead/selected-clip/timeline-end placement, use an existing compatible track, or create a new top video/audio track. Defaults to previewOnly; applying is undoable in Velorn.',
      inputSchema: {
        type: 'object',
        properties: {
          assetId: {
    ...
    },
    {
      ...
      description: 'Preview or replace an existing video/image/audio timeline clip with another project asset while preserving its timeline slot, transform, label color, effects, and keyframes by default. The old asset is not deleted. Defaults to previewOnly and is undoable in Velorn.',
      inputSchema: {
        type: 'object',
        properties: {
          clipId: { type: 'string', description: 'Timeline clip ID to replace. If omitted, Velorn must have exactly one selected clip.' },
    ...
    },
    {
      ...
      description: 'Preview or create a Velorn adjustment clip on a video track. Adjustment clips apply color/blur/GLSL/keyframed effects to clips below them. Use previewOnly first; applying is undoable in Velorn.',
      inputSchema: {
        type: 'object',
        properties: {
    ...
    },
    {
      ...
      description: 'Preview or place multiple project assets onto the active timeline as review lanes or a sequential run. Can target explicit asset IDs/names or the latest matching generated assets, create one new top track per asset, or place sequentially on one compatible track. Defaults to previewOnly; applying is undoable in Velorn.',
      inputSchema: {
        type: 'object',
        properties: {
          assets: {
    ...
    {
      name: 'duplicate_clip',
      description: 'Duplicate an existing timeline clip onto the same or another compatible track, preserving clip style, transform, effects, and keyframes. Useful for layered text/title effects. Undoable in Velorn; use previewOnly first.',
      inputSchema: {
        type: 'object',
        properties: {
          clipId: {
    ...
    {
      name: 'add_text_clip',
      description: 'Create a Velorn text clip on a video track, optionally with typography, transform, timing, preset animation, or explicit transform/text-color keyframes. Undoable in Velorn.',
      inputSchema: {
        type: 'object',
        properties: {
          ...
          animationPreset: { type: 'string', enum: ['fade', 'slideUp', 'slideDown', 'slideLeft', 'pop', 'spinIn', 'none'], description: 'Optional existing Velorn text animation preset.' },
    ...
    {
      name: 'add_shape_clip',
      description: 'Create a Velorn shape clip on a video track for motion graphics: rectangle, rounded rectangle, ellipse, polygon, or line. Use polygon with sides=3 for triangles, 6 for hexagons, 8 for octagons, etc. Supports solid, linear-gradient, and radial-gradient fills, stroke styling, transform, crop, blur, 2.5D rotation, and explicit visual keyframes including width, height, fillOpacity, gradient angle/center/radius, strokeWidth, strokeOpacity, cornerRadius, and polygon sides. Undoable in Velorn.',
      inputSchema: {
        type: 'object',
        properties: {
    ...
    },
    {
      ...
      description: 'List the GLSL/GPU-backed clip effects Velorn can add through MCP, including supported parameter keys, ranges, defaults, and presets. Use this before adding or changing GLSL effects.',
      inputSchema: {
        type: 'object',
        properties: {
    ...
    },
    {
      ...
      description: 'Add a GLSL/GPU-backed effect to a visual clip. Supports presets, static parameter settings, and optional effect-parameter keyframes. Defaults to previewOnly; applying is undoable in Velorn.',
      inputSchema: {
        type: 'object',
        properties: {
    ...
    },
    {
      ...
      description: 'Start a Velorn timeline export using the existing hidden export worker. Defaults to MP4 H.264 1080p HD with AAC audio, written to the project renders folder. Use check_export_readiness first for final delivery.',
      inputSchema: {
        type: 'object',
        properties: {
          target: {
...
class ComfyStudioMcpServer {
  ...
  async runComfyUIConnectionDiagnosis(args = {}) {
    if (!this.diagnoseComfyUIConnection) {
      return errorResult('ComfyUI connection diagnostics are not available. Restart Velorn and try again.')
    }
    try {
  ...
  async setComfyUIConnectionTool(args = {}) {
    if (!this.setComfyUIConnection) {
      return errorResult('ComfyUI connection setting is not available. Restart Velorn and try again.')
    }
  ...
  async repairComfyUIConnection(args = {}) {
    if (!this.diagnoseComfyUIConnection || !this.setComfyUIConnection) {
      return errorResult('ComfyUI connection repair is not available. Restart Velorn and try again.')
    }
    ...
    if (!reachable) {
      return textResult({
        ...
        needed: true,
        repaired: false,
        summary: 'Velorn is pointed at an unreachable ComfyUI port, and no reachable ComfyUI server was found on the probed local ports.',
        configured,
        probes,
        recommendations: [
    ...
    return textResult({
      ...
      summary: previewOnly
        ? `Velorn is pointed at ${configured?.connection?.httpBase}, but ComfyUI is reachable at ${reachable?.connection?.httpBase}. This repair would switch Velorn to port ${reachablePort}.`
        : `Velorn was switched to the reachable ComfyUI port ${reachablePort}.`,
      configured,
      reachable,
      probes,
      proposedChange: {
  ...
  getComfySetupInstallSteps(installType = 'unknown', diagnosis = null) {
    ...
    const commonFirstStep = `Velorn connects to ComfyUI by local URL, usually http://127.0.0.1:${configuredPort}. The important thing is that the port ComfyUI prints must match Settings > ComfyUI Connection in Velorn.`
    ...
    if (mode === 'portable') {
      return [
        ...
        'Use that port in Velorn Settings > ComfyUI Connection, or ask the MCP agent to apply the suggested port fix.',
        'If Velorn launches ComfyUI for you, configure Settings > ComfyUI Launcher to point at that same .bat file.',
      ]
    }
    ...
    if (mode === 'desktop') {
      return [
        ...
        'Set the same port in Velorn Settings > ComfyUI Connection.',
    ...
    if (mode === 'docker') {
      return [
        ...
        'From Windows/macOS, Velorn should still connect to localhost/127.0.0.1 on the published host port, not the container internal IP.',
        'If ComfyUI returns HTTP 403, relaunch it with a permissive CORS/header setup or use the Velorn launcher path when possible.',
      ]
    }
    ...
    if (mode === 'manual') {
      return [
        commonFirstStep,
        'Start ComfyUI manually from your terminal before opening or using Velorn generation features.',
        'Look for the URL ComfyUI prints, usually http://127.0.0.1:8188.',
        'If you deliberately use another port, set that exact port in Velorn Settings > ComfyUI Connection.',
        'If Velorn can see /system_stats but not /object_info, update ComfyUI and check custom-node import errors in the ComfyUI terminal.',
      ]
    }
    ...
    return [
      ...
      'If Velorn is on the wrong port, the MCP guide can propose the exact port change and apply it only after approval.',
  ...
  buildComfySetupUserMessage({ configured, reachable, probes, installType, previewOnly, appliedFix, validation }) {
    ...
    if (configured?.connection?.ok) {
      ...
      return [
        `Good news: Velorn is already connected to ComfyUI at ${configuredBase}.`,
        ...
        'You can now use local ComfyUI workflows from Velorn.',
      ].filter(Boolean)
    }

    if (reachable) {
      return [
        `I found ComfyUI running at ${reachableBase}, but Velorn is currently pointed at ${configuredBase}.`,
        previewOnly
          ? `No settings were changed. The safe fix is to set Velorn's ComfyUI port to ${reachable.connection.port}.`
          : appliedFix
            ? `I updated Velorn to use ${reachableBase}.`
  ...
  async guideComfyUISetup(args = {}) {
    if (!this.diagnoseComfyUIConnection) {
      return errorResult('ComfyUI setup guidance is not available. Restart Velorn and try again.')
    }
    ...
    const summary = connected
      ? 'Velorn is connected to ComfyUI.'
      : reachable
        ? 'ComfyUI is running, but Velorn needs a port-setting fix.'
  ...
  async controlComfyUILauncherTool(args = {}) {
    if (!this.controlComfyLauncher) {
      return errorResult('ComfyUI launcher control is not available. Restart Velorn and try again.')
    }
  ...
  async getComfyUILauncherLogsTool(args = {}) {
    if (!this.getComfyLauncherLogs) {
      return errorResult('ComfyUI launcher logs are not available. Restart Velorn and try again.')
    }
    try {
  ...
  async runComfyUINodeValidation(args = {}) {
    if (!this.validateComfyUINodes) {
      return errorResult('ComfyUI node validation is not available. Restart Velorn and try again.')
    }
    try {
  ...
  async listComfyStudioWorkflowsTool(args = {}) {
    if (!this.listComfyStudioWorkflows) {
      return errorResult('Velorn workflow discovery is not available. Restart Velorn and try again.')
    }
    try {
  ...
  async inspectComfyStudioWorkflowTool(args = {}) {
    if (!this.inspectComfyStudioWorkflow) {
      return errorResult('Velorn workflow inspection is not available. Restart Velorn and try again.')
    }
    try {
      ...
      if (result?.success === false) return errorResult(result.error || 'Could not inspect Velorn workflow.')
      return textResult(result)
    } catch (error) {
      return errorResult(`Could not inspect Velorn workflow: ${error?.message || String(error)}`)
    }
  }
  ...
  async runRendererActionTool(action, args = {}, options = {}) {
    ...
    if (!this.performAction) {
      return errorResult(`${bridgeName} is not available. Restart Velorn and try again.`)
    }
    const payload = { ...(args || {}) }
    ...
    try {
      ...
      return textResult({
        ...
        message: result?.message || (previewOnly
          ? `${suggestedTool} plan returned by Velorn.`
          : `${suggestedTool} applied through Velorn.`),
        result,
  ...
  async runMcpActionPlan(snapshot, args = {}) {
    ...
    if (!this.performAction) {
      return errorResult('MCP action-plan bridge is not available. Restart Velorn and try again.')
    }
  ...
  async prepareGenerationFromTimelineContext(snapshot, args = {}) {
    ...
    if (!this.performAction) {
      return errorResult('MCP Generate preparation bridge is not available. Restart Velorn and try again.')
    }
    ...
    return textResult({
      success: true,
      action: 'prepare_generation_from_timeline_context',
      message: 'Timeline frame captured and sent to Generate. Review settings in Velorn, then click Generate manually when ready.',
      plan: {
        ...plan,
        previewOnly: false,
      },
  ...
  async queuePreparedGeneration(_snapshot, args = {}) {
    if (!this.performAction) {
      return errorResult('MCP Generate queue bridge is not available. Restart Velorn and try again.')
    }
    ...
    return textResult({
      ...
      message: previewOnly
        ...
        : 'Prepared Generate request queued through Velorn.',
      result,
    })
  }
  ...
  async queueTimelineGenerationBatch(snapshot, args = {}) {
    ...
    if (!this.performAction) {
      return errorResult('MCP timeline generation batch bridge is not available. Restart Velorn and try again.')
    }
    ...
    return textResult({
      ...
      previewOnly: false,
      action: 'queue_timeline_generation_batch',
      message: `Queued ${plan.totalJobs} timeline generation job${plan.totalJobs === 1 ? '' : 's'} through Velorn.`,
      plan,
      result,
    })
  }
  ...
  async queueTimelineTemplateGeneration(snapshot, args = {}) {
    ...
    if (!this.performAction) {
      return errorResult('MCP template generation bridge is not available. Restart Velorn and try again.')
    }
    ...
    return textResult({
      ...
      previewOnly: false,
      action: 'queue_timeline_template_generation',
      message: `Queued "${plan.template?.title || plan.template?.name || 'ComfyUI template'}" from the timeline source clip through Velorn.`,
      plan,
      result,
    })
  }
  ...
  async queuePromptGenerationBatch(snapshot, args = {}) {
    ...
    if (!this.performAction) {
      return errorResult('MCP prompt generation batch bridge is not available. Restart Velorn and try again.')
    }
    ...
    return textResult({
      ...
      previewOnly: false,
      action: 'queue_prompt_generation_batch',
      message: `Queued ${plan.totalJobs} prompt generation job${plan.totalJobs === 1 ? '' : 's'} through Velorn.`,
      plan,
      result,
    })
  }
  ...
  async inspectTimelineFrame(snapshot, args = {}) {
    ...
    if (includeImage) {
      if (!this.performAction) {
        warning = 'MCP frame capture bridge is not available. Restart Velorn and try again.'
      } else {
        try {
  ...
  async setClipLabelColor(snapshot, args = {}) {
    ...
    if (!this.performAction) {
      return errorResult('MCP write bridge is not available. Restart Velorn and try again.')
    }
  ...
  async createProject(snapshot, args = {}) {
    if (!this.performAction) {
      return errorResult('MCP project bridge is not available. Restart Velorn and try again.')
    }
  ...
  async addTrack(snapshot, args = {}) {
    ...
    if (!this.performAction) {
      return errorResult('MCP track bridge is not available. Restart Velorn and try again.')
    }
  ...
  async updateTrack(snapshot, args = {}) {
    ...
    if (!this.performAction) return errorResult('MCP track bridge is not available. Restart Velorn and try again.')
    const result = await this.performAction({
      action: 'update_track',
  ...
  async switchTimeline(snapshot, args = {}) {
    ...
    if (!this.performAction) return errorResult('MCP timeline bridge is not available. Restart Velorn and try again.')
    const result = await this.performAction({
      action: 'switch_timeline',
  ...
  async addTransition(snapshot, args = {}) {
    ...
    if (!this.performAction) return errorResult('MCP transition bridge is not available. Restart Velorn and try again.')
    const result = await this.performAction({
      action: 'add_transition',
  ...
  async moveClips(snapshot, args = {}) {
    ...
    if (!this.performAction) return errorResult('MCP clip edit bridge is not available. Restart Velorn and try again.')
    const result = await this.performAction({
      action: 'move_clips',
      payload: { ...args, previewOnly: false },
    })
  ...
  async createTimeline(snapshot, args = {}) {
    ...
    if (!this.performAction) {
      return errorResult('MCP sequence creation bridge is not available. Restart Velorn and try again.')
    }
    ...
    return textResult({
      ...
      message: plan.switchToTimeline
        ? 'Sequence created and Velorn switched to it.'
        : 'Sequence created.',
      plan,
      result,
    })
  ...
  async createAssetFolder(snapshot, args = {}) {
    ...
    if (!this.performAction) {
      return errorResult('MCP asset folder bridge is not available. Restart Velorn and try again.')
    }
  ...
  async moveAssetsToFolder(snapshot, args = {}) {
    ...
    if (!this.performAction) {
      return errorResult('MCP asset move bridge is not available. Restart Velorn and try again.')
    }
  ...
  async moveUnusedAssetsToFolder(_snapshot, args = {}) {
    if (!this.performAction) {
      return errorResult('MCP unused asset cleanup bridge is not available. Restart Velorn and try again.')
    }
    ...
    try {
      ...
      return textResult({
        ...
        action: 'move_unused_assets_to_folder',
        message: result?.message || (previewOnly
          ? 'Unused asset cleanup plan returned by Velorn.'
          : 'Unused assets moved by Velorn.'),
        result,
        suggestedApplyCall: previewOnly ? {
  ...
  async addAssetToTimeline(snapshot, args = {}) {
    ...
    if (!this.performAction) {
      return errorResult('MCP asset placement bridge is not available. Restart Velorn and try again.')
    }
    ...
    return textResult({
      success: true,
      action: 'add_asset_to_timeline',
      message: 'Asset placed on the timeline through Velorn.',
      plan,
      result,
    })
  }
  ...
  async replaceClipWithAsset(_snapshot, args = {}) {
    if (!this.performAction) {
      return errorResult('MCP clip replacement bridge is not available. Restart Velorn and try again.')
    }
    ...
    try {
      ...
      return textResult({
        ...
        action: 'replace_clip_with_asset',
        message: result?.message || (previewOnly
          ? 'Clip replacement plan returned by Velorn.'
          : 'Clip replaced through Velorn.'),
        result,
        suggestedApplyCall: previewOnly ? {
  ...
  async addSolidColor(snapshot, args = {}) {
    ...
    if (!this.performAction) {
      return errorResult('MCP solid color bridge is not available. Restart Velorn and try again.')
    }
    ...
    return textResult({
      success: true,
      action: 'add_solid_color',
      message: 'Solid color asset created through Velorn.',
      plan,
      result,
    })
  }
  ...
  async addAssetsToTimeline(snapshot, args = {}) {
    ...
    if (!this.performAction) {
      return errorResult('MCP batch asset placement bridge is not available. Restart Velorn and try again.')
    }
    ...
    return textResult({
      success: true,
      action: 'add_assets_to_timeline',
      message: 'Assets placed on the timeline through Velorn.',
      plan,
      result,
    })
  }
  ...
  async addAdjustmentClip(snapshot, args = {}) {
    ...
    if (!this.performAction) {
      return errorResult('MCP adjustment clip bridge is not available. Restart Velorn and try again.')
    }
    ...
    return textResult({
      success: true,
      action: 'add_adjustment_clip',
      message: 'Adjustment clip created through Velorn.',
      plan,
      result,
    })
  }
  ...
  async duplicateClip(snapshot, args = {}) {
    ...
    if (!this.performAction) {
      return errorResult('MCP duplicate bridge is not available. Restart Velorn and try again.')
    }
  ...
  async addTextClip(snapshot, args = {}) {
    ...
    if (!this.performAction) {
      return errorResult('MCP text bridge is not available. Restart Velorn and try again.')
    }
  ...
  async addShapeClip(snapshot, args = {}) {
    ...
    if (!this.performAction) {
      return errorResult('MCP shape bridge is not available. Restart Velorn and try again.')
    }
  ...
  async listGlslEffects(args = {}) {
    ...
    return textResult({
      warning: 'Renderer effect registry is not available. Restart Velorn for full parameter details.',
  ...
  async addGlslEffect(snapshot, args = {}) {
    ...
    if (!this.performAction) {
      return errorResult('MCP effect bridge is not available. Restart Velorn and try again.')
    }
  ...
  async setClipKeyframes(snapshot, args = {}) {
    ...
    if (!this.performAction) {
      return errorResult('MCP clip keyframe bridge is not available. Restart Velorn and try again.')
    }
    ...
    return textResult({
      success: true,
      action: 'set_clip_keyframes',
      message: 'Clip keyframes updated through Velorn.',
      plan,
      result,
    })
  }
  ...
  async addDipToBlack(_snapshot, args = {}) {
    if (!this.performAction) {
      return errorResult('MCP dip-to-black bridge is not available. Restart Velorn and try again.')
    }
    ...
    try {
      ...
      return textResult({
        ...
        action: 'add_dip_to_black',
        message: result?.message || (previewOnly
          ? 'Dip-to-black opacity keyframe plan returned by Velorn.'
          : 'Dip-to-black opacity keyframes applied through Velorn.'),
        result,
        suggestedApplyCall: previewOnly ? {
  ...
  async exportTimeline(snapshot, args = {}) {
    ...
    if (!this.performAction) {
      return errorResult('MCP export bridge is not available. Restart Velorn and try again.')
    }
    ...
    return textResult({
      success: true,
      action: 'export_timeline',
      message: 'Export started in the Velorn export worker.',
      ...exportPlan,
      result,
    })
  }
  ...
  async exportDeliveryBatch(snapshot, args = {}) {
    ...
    if (!this.performAction) {
      return errorResult('MCP batch export bridge is not available. Restart Velorn and try again.')
    }
    ...
    return textResult({
      ...
      action: 'export_delivery_batch',
      message: failedCount === 0
        ? `Started ${results.length} delivery export${results.length === 1 ? '' : 's'} through Velorn.`

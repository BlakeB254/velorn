...
function getClipVisualSource(snapshot, clip, asset) {
  ...
  if (posterPath) {
    return {
      ...
      description: 'Representative poster frame generated by CDX Studio.',
    }
  }
...
function getEffectClipForMcp(snapshot, clipId) {
  ...
  if (!id) return { error: 'Provide clipId from get_timeline, or select exactly one visual clip in CDX Studio.' }
...
function resolveSolidColorPlan(snapshot, args = {}) {
  ...
  if (!timeline || !project) return { error: 'Open a saved CDX Studio project and timeline before creating a solid color.' }
...
function resolveCreateTimelinePlan(snapshot, args = {}) {
  const project = snapshot?.project || null
  if (!project) return { error: 'Open a saved CDX Studio project before creating a sequence.' }
...
function resolveCreateAssetFolderPlan(snapshot, args = {}) {
  const project = snapshot?.project || null
  if (!project) return { error: 'Open a saved CDX Studio project before creating an asset folder.' }
...
function resolveMoveAssetsToFolderPlan(snapshot, args = {}) {
  const project = snapshot?.project || null
  if (!project) return { error: 'Open a saved CDX Studio project before moving assets.' }
  ...
  return [
    {
      ...
      mimeType: 'text/plain',
      description: 'CDX Studio production types CDX Studio accepts: show, commercial/advertisement, music-video, ig-short, skit, movie, psa, animated, website-tour, hype-video, documentary, …',
    },
    {
...
function buildAiReviewPasses(snapshot) {
  ...
  return {
    purpose: 'Use these recipes as safe AI review passes for the open CDX Studio project. Prefer previewOnly before write actions that alter clips or markers.',
    timeline: timeline ? {
      id: timeline.id,
      name: timeline.name,
    ...
    recipes: [
      ...
      },
      {
        ...
        goal: 'Create or duplicate a CDX Studio project before risky AI edits or fresh automated layouts.',
      ...
      },
      {
        ...
        goal: 'Add, update, or remove CDX Studio native transitions such as dissolves, fades, wipes, slides, zooms, blur, and dip-to-black style edits.',
      ...
      },
      {
        ...
        goal: 'Turn a written creative brief into source images or videos that can be assembled into a new CDX Studio sequence.',
      ...
      },
      {
        ...
        goal: 'Create a complete, editable Music Video through a collaborative, approval-based conversation using the same Director state as the CDX Studio UI.',
      ...
      },
      {
        ...
        goal: 'Export the active CDX Studio timeline as FCPXML for Resolve/Final Cut or XMEML v5 for Premiere Pro.',
...
function resolveExportDeliveryPlan(snapshot, args = {}) {
  ...
  const project = snapshot?.project || null
  if (!project || !timeline) {
    return { error: 'Open a saved CDX Studio project and timeline before exporting.' }
  }
  ...
  const safeProjectName = sanitizeExportBaseName(project.name || 'CDX Studio')
...
function checkExportReadiness(snapshot, args = {}) {
  ...
  if (plan.settings.useHardwareEncoder) notes.push('Export requests hardware encoding; CDX Studio probes the active export FFmpeg at runtime and falls back to software encoding if the requested encoder is unavailable.')
    ...
    try {
      jobs = args.jobs.map((job, index) => {
        ...
        if (workflowInfo.requiresInputImage && !assetFieldIds.image && !assetFieldIds.inputImage) {
          throw new Error(`Workflow "${workflowId}" needs an input image. Provide jobs[].assetFieldIds.image with a CDX Studio image asset id.`)
        }
        return {
          workflowId,
  ...
  if (!timeline) {
    return {
      ...
      findings: [{
        ...
        message: 'CDX Studio does not currently have an active timeline snapshot.',
      }],
    }
  }
...
function createToolDefinitions() {
  return [
    {
      name: 'get_project',
      description: 'Return a concise summary of the open CDX Studio project, current timeline, asset counts, and MCP snapshot freshness.',
      inputSchema: {
        type: 'object',
        properties: {},
      },
    ...
    },
    {
      ...
      description: 'Return practical AI review recipes for CDX Studio MCP clients: timeline health, visible-shot review, hero presence checks, marker cleanup, disabled clip labeling, clip enable/disable previews, delivery checks, and current-frame questions.',
      inputSchema: {
        type: 'object',
        properties: {},
      },
    },
    {
      ...
      description: 'Return practical MCP agent recipes and recommended workflows for CDX Studio. This is a clearer alias for get_ai_review_passes and should be used when a user asks what the MCP can do.',
      inputSchema: {
        type: 'object',
        properties: {},
      },
    },
    {
      ...
      description: 'Return the layered CDX Studio production packet: show → season → episode → shot, plus cast, locations (plates/depth/blender), storyboard/sequence have-vs-missing, film lexicon, and optional camera xyz handles. Start here for any show or episode job.',
      inputSchema: {
        type: 'object',
        properties: {
    ...
    },
    {
      ...
      description: 'Resource discovery for CDX Studio video work. Given a type (commercial, advertisement, music-video, show, ig-short, …) or a free-text query, return the canonical type, pacing defaults, skills to load, and the MCP flow/tools to run. Works without an open project.',
      inputSchema: {
        type: 'object',
        properties: {
          type: { type: 'string', description: 'CDX/CDX Studio production type or alias (advertisement, ad, film, mv, …).' },
    ...
    },
    {
      ...
      inputSchema: {
        ...
        properties: {
          ...
          outputTarget: { type: 'string', enum: ['mobile', 'computer', 'square', 'portrait-feed'], description: 'Project output ratio: mobile 9:16, computer 16:9, square 1:1, portrait-feed 4:5. Updates the edit canvas and generate size.' },
          format: { type: 'object', description: 'format.outputTarget / format.aspect / format.fps' },
    ...
    },
    {
      ...
      inputSchema: {
        ...
        properties: {
          ...
          outputTarget: { type: 'string', enum: ['mobile', 'computer', 'square', 'portrait-feed'], description: 'Override this shot\'s output ratio. Empty/omit keeps the project default.' },
          previewOnly: { type: 'boolean' },
        },
        required: ['cardId'],
      },
    ...
    {
      name: 'studio_audit',
      description: 'Join slots, clips, QA verdicts, rubrics, and dialogue into per-shot Studio verdicts (NEEDS_REGEN / READY_TO_GENERATE / DIALOGUE_BLOCKED / NEEDS_FLF / DONE). DONE ≠ PASSED. Read-only. Uses CDX Studio project data, not cdx-video-director.',
      inputSchema: {
        type: 'object',
        properties: {
    ...
    {
      name: 'studio_flow',
      description: 'Return the stage rail: script → cast → scenes → storyboard → flf → video → review → edit → deliver, with stage_reached and stage_blocked_at. Includes take-chain readiness and the VSE audio plan. Also returns the shot routing matrix for every card/slot (script call → CDX Studio workflow).',
      inputSchema: { type: 'object', properties: {} },
    },
    {
      name: 'studio_route_shot',
      description: 'Classify a shot description (or an open-project card) into the CDX Studio shot-routing matrix and return the generation choice: ecosystem, bundle, still/video/draft workflow ids. Does not queue GPU work. GPU serial; drafts-only outward.',
      inputSchema: {
        type: 'object',
        properties: {
    ...
    },
    {
      ...
      description: 'List CDX Studio voice profiles. Studio Qwen3/Chatterbox map onto the ElevenLabs TTS workflow. blake-recorded means do not TTS.',
      inputSchema: { type: 'object', properties: {} },
    },
    {
    ...
    },
    {
      ...
      description: 'Plan or persist a TTS take for a line. Defaults to previewOnly. Does not queue GPU. CDX Studio lane is elevenlabs-tts; outward stays draft.',
      inputSchema: {
        type: 'object',
        properties: {
          lineSlug: { type: 'string' },
    ...
    },
    {
      ...
      description: 'List or get CDX Studio animation/film style cards (studio_animation_styles parity). Read-only catalog.',
      inputSchema: {
        type: 'object',
        properties: {
    ...
    },
    {
      ...
      description: 'CDX Studio-native Core trace bridge. Plan or record a generation/feedback/MCP ledger write with skill_version_id. Defaults to previewOnly. Does not depend on CDX Studio.',
      inputSchema: {
        type: 'object',
        properties: {
    ...
    },
    {
      ...
      description: 'Diagnose the local ComfyUI connection used by CDX Studio. Checks the configured localhost port, ComfyUI API endpoints, launcher state, port owner, likely install mode such as portable/Desktop/Docker/manual, and returns support-friendly next steps.',
      inputSchema: {
        type: 'object',
        properties: {
          port: { type: 'integer', description: 'Optional local ComfyUI port to test. Defaults to CDX Studio Settings > ComfyUI Connection.' },
    ...
    },
    {
      ...
      description: 'Set the local ComfyUI connection port used by CDX Studio. Supports previewOnly so assistants can propose the change before applying it. This changes CDX Studio settings only; it does not restart ComfyUI or edit launcher scripts.',
      inputSchema: {
        type: 'object',
        properties: {
          port: { type: 'integer', description: 'Local ComfyUI port to save, for example 8188.' },
          previewOnly: { type: 'boolean', description: 'When true, returns the proposed before/after setting without changing CDX Studio.' },
        },
        required: ['port'],
      },
    },
      ...
      inputSchema: {
        ...
        properties: {
          strategy: {
            ...
            description: 'Repair strategy. useReachablePort finds a reachable local ComfyUI port and points CDX Studio at it.',
          },
          candidatePorts: {
            type: 'array',
            items: { type: 'integer' },
          ...
          previewOnly: { type: 'boolean', description: 'When true, returns the proposed repair without changing CDX Studio. Defaults to true.' },
        },
      },
    },
    {
      name: 'guide_comfyui_setup',
      description: 'Beginner-friendly CDX Studio-to-ComfyUI setup wizard. Diagnoses the current connection, probes common local ComfyUI ports, explains what to do for Portable/Desktop/Docker/manual installs, and can preview or apply the safe port-setting fix when ComfyUI is found on a different port.',
      inputSchema: {
        type: 'object',
        properties: {
          installType: {
          ...
          applyFix: {
            type: 'boolean',
            description: 'When true and previewOnly=false, update CDX Studio to the reachable ComfyUI port if a safe port mismatch is found. Defaults to false.',
          },
          previewOnly: {
            type: 'boolean',
            description: 'When true, diagnose and propose the setup/fix without changing CDX Studio. Defaults to true.',
          },
          nodeClasses: {
            type: 'array',
            items: { type: 'string' },
    ...
    },
    {
      ...
      description: 'Preview or apply ComfyUI launcher actions through CDX Studio: start, stop, or restart. Defaults to previewOnly for safety. Stop/restart can interrupt running generations and only work for ComfyUI processes owned by CDX Studio.',
      inputSchema: {
        type: 'object',
        properties: {
          action: {
    ...
    },
    {
      ...
      description: 'Return recent ComfyUI launcher log lines from CDX Studio, with a lightweight summary of common support issues like port conflicts, import errors, missing models/files, and CUDA memory errors.',
      inputSchema: {
        type: 'object',
        properties: {
    ...
    },
    {
      ...
      description: 'List bundled CDX Studio workflows and graphs saved under Generate > My Workflows. Saved graphs include MCP readiness, detected CDX Studio markers, output type, and any setup blockers. Ready My Workflows entries can be passed to queue_prompt_generation_batch by their my-workflow: id.',
      inputSchema: {
        type: 'object',
        properties: {
          runtime: {
    ...
    },
    {
      ...
      description: 'Inspect a bundled, My Workflows, or explicit CDX Studio workflow JSON. Returns MCP readiness and marker requirements for saved graphs, extracts ComfyUI node classes, validates them against local /object_info, and provides setup hints.',
      inputSchema: {
        type: 'object',
        properties: {
          workflowId: {
          ...
          port: {
            type: 'integer',
            description: 'Optional local ComfyUI port to test. Defaults to CDX Studio Settings > ComfyUI Connection.',
          },
          timeoutMs: {
            type: 'integer',
    ...
    },
    {
      ...
      inputSchema: {
        ...
        properties: {
          timeSeconds: {
            type: 'number',
            description: 'Optional timeline time in seconds. Defaults to the current CDX Studio playhead.',
          },
          frame: {
            type: 'integer',
    ...
    },
    {
      ...
      inputSchema: {
        ...
        properties: {
          ...
          workflowId: {
            type: 'string',
            description: 'CDX Studio workflow id to select in Generate. Defaults to ltx23-i2v.',
          },
          category: {
            type: 'string',
          ...
          prompt: {
            type: 'string',
            description: 'Prompt to prefill. If omitted, CDX Studio tries to reuse prompt metadata from the source clip/asset.',
          },
          negativePrompt: {
            type: 'string',
    ...
    },
    {
      ...
      description: 'Queue the generation currently staged in the Generate tab by using the same queue path as CDX Studio’s Queue button. Use previewOnly first. Applying can start local/credit-backed generation depending on the selected workflow, so only call with previewOnly=false after explicit user approval.',
      inputSchema: {
        type: 'object',
        properties: {
          previewOnly: {
    ...
    },
    {
      ...
      inputSchema: {
        ...
        properties: {
          ...
          prompt: {
            type: 'string',
            description: 'Prompt for every variation. If omitted, CDX Studio tries to reuse prompt metadata from the source clip/asset.',
          },
          negativePrompt: {
            type: 'string',
    ...
    },
    {
      ...
      inputSchema: {
        ...
        properties: {
          ...
          seed: {
            type: 'integer',
            description: 'Optional seed. If omitted, CDX Studio generates one.',
          },
          durationSeconds: {
            type: 'number',
          ...
          assetFieldIds: {
            type: 'object',
            description: 'Optional map of imported template assetSelect field IDs to CDX Studio asset IDs, for templates with required extra media inputs.',
            additionalProperties: { type: 'string' },
          },
          folderId: {
            type: 'string',
            description: 'Optional existing CDX Studio asset folder ID where generated result assets should be organized after import.',
          },
          outputFolderId: {
            type: 'string',
    ...
    },
    {
      ...
      inputSchema: {
        ...
        properties: {
          ...
          title: {
            type: 'string',
            description: 'Human title shown in CDX Studio.',
          },
          modelUrls: {
            type: 'array',
            maxItems: 20,
            items: {
              ...
              properties: {
                ...
                targetSubdir: { type: 'string', description: 'ComfyUI models/ subfolder (for example "loras"). Needed when CDX Studio cannot infer it.' },
              },
    ...
    },
    {
      ...
      description: 'Preview or queue one MiniMax H3 reference-to-video job from an exact CDX Studio image asset and audio asset. Designed for paid hero performance and lip-sync shots. Defaults to previewOnly; applying can spend Comfy credits, so require explicit approval first. No negative prompt is sent.',
      inputSchema: {
        type: 'object',
        required: ['imageAssetId', 'audioAssetId', 'prompt'],
        properties: {
          imageAssetId: { type: 'string', description: 'CDX Studio image asset ID for the exact first/reference frame.' },
          audioAssetId: { type: 'string', description: 'CDX Studio audio asset ID for the exact performance segment.' },
          ...
          folderId: { type: 'string', description: 'Optional CDX Studio asset-folder ID for the completed result.' },
    ...
    },
    {
      ...
      description: 'Return live CDX Studio Generate-queue jobs, including queued/running/completed/failed state, progress, prompt IDs, and imported result asset IDs. Filter by workflowId, jobIds, or batchId. Read-only.',
      inputSchema: {
        type: 'object',
        properties: {
          workflowId: { type: 'string' },
    ...
    },
    {
      ...
      inputSchema: {
        ...
        properties: {
          ...
          jobs: {
            ...
            items: {
              ...
              properties: {
                ...
                assetFieldIds: {
                  type: 'object',
                  description: 'Map workflow asset fields to CDX Studio asset IDs, for example { "referenceImage1": "...", "referenceImage2": "..." } for Seedance 2.0 R2V, or { "image": "<image asset id>" } for input-image workflows such as image-edit.',
                  additionalProperties: { type: 'string' },
                },
                referenceImages: {
                  type: 'array',
          ...
          folderId: {
            type: 'string',
            description: 'Optional existing CDX Studio asset folder ID where generated result assets should be organized after import. Use create_asset_folder first when a new folder is needed.',
          },
          outputFolderId: {
            type: 'string',
            description: 'Alias for folderId.',
    ...
    },
    {
      ...
      inputSchema: {
        ...
        properties: {
          startSeconds: {
            type: 'number',
            description: 'Optional range start in seconds. Defaults to the current CDX Studio playhead.',
          },
          endSeconds: {
            type: 'number',
    ...
    },
    {
      ...
      description: 'List recently opened CDX Studio projects (name, path, last modified, whether currently open). Works with no project open. Use with open_project to recover after an app restart or switch projects.',
      inputSchema: {
        type: 'object',
        properties: {
    ...
    {
      name: 'open_project',
      description: 'Open a CDX Studio project by folder path or by recent-project name. Works with no project open; replaces the currently open project (work is autosaved). Defaults to previewOnly.',
      inputSchema: {
        type: 'object',
        properties: {
    ...
    },
    {
      ...
      description: 'Preview or queue Music Video keyframes for missing, all, or selected shots using native Director routing. Reference-free b-roll still follows CDX Studio local fallback rules. Applying may start local GPU work or spend cloud credits. Defaults to previewOnly.',
      inputSchema: {
        type: 'object',
        properties: {
    ...
    {
      name: 'save_project',
      description: 'Preview or explicitly save the current CDX Studio project, including Director state, assets, and the active timeline. Applied MCP writes also autosave this live file so Blake can open the folder and see current state. Defaults to previewOnly.',
      inputSchema: {
        type: 'object',
        properties: {
          previewOnly: { type: 'boolean', description: 'Defaults to true.' },
    ...
    },
    {
      ...
      description: 'Summarize CDX Studio music-video workflow assets, assembled clips, and sync-locked clips in the current project.',
      inputSchema: {
        type: 'object',
        properties: {},
      },
    ...
    },
    {
      ...
      description: 'Preview or queue regeneration of one Music Video Step 4 shot using the exact active keyframe settings and native CDX Studio routing. This includes reference-free b-roll fallback routing. Defaults to previewOnly and may start local GPU work or spend cloud credits when applied.',
      inputSchema: {
        type: 'object',
        properties: {
          sceneId: { type: 'string', description: 'Exact scene ID from the parsed Music Video director script, such as S1.' },
    ...
    },
    {
      ...
      description: 'Preview or queue regeneration of one Music Video Step 5 shot using its generated keyframe and the exact active video settings and native CDX Studio routing. Defaults to previewOnly and may start local GPU work or spend cloud credits when applied.',
      inputSchema: {
        type: 'object',
        properties: {
          sceneId: { type: 'string', description: 'Exact scene ID from get_music_video_plan, such as S1.' },
    ...
    },
    {
      ...
      description: 'Set or clear label colors for current timeline clips by explicit clipIds or a safe built-in filter. Non-destructive and undoable in CDX Studio.',
      inputSchema: {
        type: 'object',
        properties: {
          color: {
    ...
    },
    {
      ...
      description: 'Enable or disable current timeline clips by explicit clipIds or a safe built-in filter. Use previewOnly first for AI-assisted edit decisions. Undoable in CDX Studio.',
      inputSchema: {
        type: 'object',
        properties: {
          enabled: {
    ...
    },
    {
      ...
      description: 'Add one or more labeled timeline markers at explicit times, frames, or the current playhead. Non-destructive and undoable in CDX Studio.',
      inputSchema: {
        type: 'object',
        properties: {
          markers: {
    ...
    },
    {
      ...
      description: 'Remove timeline markers by ID, all markers, marker color, label text, or time range. Undoable in CDX Studio; use previewOnly for safety.',
      inputSchema: {
        type: 'object',
        properties: {
          all: {
    ...
    },
    {
      ...
      description: 'Rename, recolor, or move existing timeline markers by ID, color, label text, or time range. Undoable in CDX Studio; use previewOnly before changing many review markers.',
      inputSchema: {
        type: 'object',
        properties: {
          all: {
    ...
    {
      name: 'undo',
      description: 'Undo the latest CDX Studio timeline edit or sequence/project-structure edit. Scope can be auto, timeline, or project. Low-risk and mirrors the app undo stack.',
      inputSchema: {
        type: 'object',
        properties: {
    ...
    {
      name: 'redo',
      description: 'Redo the latest CDX Studio timeline edit or sequence/project-structure edit. Scope can be auto, timeline, or project. Low-risk and mirrors the app redo stack.',
      inputSchema: {
        type: 'object',
        properties: {
    ...
    {
      name: 'select_assets',
      description: 'Select/preview project assets by ID, name, type, folder, status, or latest match. CDX Studio currently previews the first matched asset.',
      inputSchema: {
        type: 'object',
        properties: {
    ...
    },
    {
      ...
      description: 'Preview or restore an in-memory MCP project checkpoint created during this CDX Studio session. Defaults to previewOnly because it can replace current timeline/asset state.',
      inputSchema: {
        type: 'object',
        properties: {
    ...
    },
    {
      ...
      description: 'Preview or import a local media path into the active CDX Studio project. Single files use the same copy/import path as the UI import button. Image sequences import too: point at a folder of numbered frames (or any one frame of a 3+ frame run — png/jpg/webp/tif/exr/dpx) and the run transcodes once into a single video clip tagged with its sequence provenance; gaps hold the previous frame.',
      inputSchema: {
        type: 'object',
        properties: {
    ...
    {
      name: 'relink_asset',
      description: 'Preview or relink an existing CDX Studio asset record to a different local file path. Useful after check_media_health finds missing media. This updates CDX Studio project metadata only; it does not copy media. Defaults to previewOnly.',
      inputSchema: {
        type: 'object',
        properties: {
    ...
    },
    {
      ...
      description: 'Preview or run a small ordered batch of approved CDX Studio MCP write actions. Use this when an agent has already planned several safe steps and wants one checkpointed apply pass. Defaults to previewOnly.',
      inputSchema: {
        type: 'object',
        properties: {
          steps: {
    ...
    {
      name: 'create_project',
      description: 'Preview or create a new CDX Studio project in the configured Projects folder. Applying opens the new empty project. Defaults to previewOnly so agents cannot accidentally create folders without approval.',
      inputSchema: {
        type: 'object',
        properties: {
          name: {
          ...
          width: {
            type: 'integer',
            description: 'Optional project width. Defaults to CDX Studio new-project defaults.',
          },
          height: {
            type: 'integer',
            description: 'Optional project height. Defaults to CDX Studio new-project defaults.',
          },
          fps: {
            type: 'number',
            description: 'Optional project frame rate. Defaults to CDX Studio new-project defaults.',
          },
          type: {
            type: 'string',
    ...
    },
    {
      ...
      description: 'Preview or duplicate a CDX Studio project folder. Defaults to the current open project, copies the whole folder, remaps saved paths, creates a sibling copy, and opens the duplicate on apply.',
      inputSchema: {
        type: 'object',
        properties: {
          sourceProjectPath: {
    ...
    },
    {
      ...
      description: 'Preview or create a new CDX Studio sequence/timeline with a name, optional dimensions, fps, duration, and optional switch-to-new-sequence behavior. Defaults to previewOnly; applying is undoable in CDX Studio.',
      inputSchema: {
        type: 'object',
        properties: {
          name: {
          ...
          switchToTimeline: {
            type: 'boolean',
            description: 'When true, switch CDX Studio to the new sequence after creation. Defaults to true.',
          },
          activate: {
            type: 'boolean',
    ...
    },
    {
      ...
      description: 'Preview or create CDX Studio asset-panel folders. Accepts a single folder name or a nested path like "AI Spots / July 4th Demo"; reuses existing matching folders by default and creates only missing folders.',
      inputSchema: {
        type: 'object',
        properties: {
          name: {
    ...
    },
    {
      ...
      description: 'Preview or move existing CDX Studio assets into an asset-panel folder. Can target explicit asset IDs/names or safe filters such as rootOnly + constantsOnly. Defaults to previewOnly and can create a missing target folder path before moving.',
      inputSchema: {
        type: 'object',
        properties: {
          targetFolderId: {
    ...
    {
      name: 'add_track',
      description: 'Create a new timeline track. Video tracks are added at the top of the stack, which is useful before adding another text/title layer. Undoable in CDX Studio.',
      inputSchema: {
        type: 'object',
        properties: {
          type: {
    ...
    {
      name: 'update_track',
      description: 'Preview or update an existing timeline track: rename it, mute/unmute, solo, lock/unlock, show/hide, set mixer volume/pan, edit insert effects, change audio channels, or reorder within its video/audio group. Undoable in CDX Studio.',
      inputSchema: {
        type: 'object',
        properties: {
    ...
    {
      name: 'remove_track',
      description: 'Preview or remove a timeline track and all clips on it. CDX Studio protects the last track of each type. Use previewOnly first.',
      inputSchema: {
        type: 'object',
        properties: {
          trackId: { type: 'string', description: 'Track ID from get_timeline.' },
    ...
    },
    {
      ...
      description: 'Preview or switch the active CDX Studio sequence/timeline. Useful when an agent creates review timelines and then needs to work inside one.',
      inputSchema: {
        type: 'object',
        properties: {
    ...
    {
      name: 'add_transition',
      description: 'Preview or add a native CDX Studio transition. Use clipAId+clipBId for a between-clips transition on the same track, or clipId+edge for an in/out edge transition. Undoable in CDX Studio.',
      inputSchema: {
        type: 'object',
        properties: {
    ...
    },
    {
      ...
      description: 'Preview or update a native transition type, duration, alignment, or settings. Undoable in CDX Studio.',
      inputSchema: {
        type: 'object',
        properties: {
    ...
    },
    {
      ...
      description: 'Preview or remove one or more native CDX Studio transitions. Undoable in CDX Studio.',
      inputSchema: {
        type: 'object',
        properties: {
    ...
    {
      name: 'move_clips',
      description: 'Preview or move one or more clips to a track and/or start time. Use for AI timeline layout cleanup, review lanes, and repositioning generated assets. Undoable in CDX Studio.',
      inputSchema: {
        type: 'object',
        properties: {
          ...
          resolveOverlaps: { type: 'boolean', description: 'When true, use CDX Studio overwrite/overlap resolution. Defaults to false.' },
    ...
    {
      name: 'trim_clips',
      description: 'Preview or update clip timing/trim values for one or more clips. Supports startSeconds, durationSeconds, trimStartSeconds, and trimEndSeconds. Undoable in CDX Studio.',
      inputSchema: {
        type: 'object',
        properties: {
    ...
    {
      name: 'delete_clips',
      description: 'Preview or delete timeline clips by ID or simple filter. Supports ripple delete. Destructive but undoable in CDX Studio; use previewOnly first.',
      inputSchema: {
        type: 'object',
        properties: {
    ...
    },
    {
      ...
      description: 'Preview or place a project asset on the active timeline. Can target a specific asset id/name or the latest generated asset, choose playhead/selected-clip/timeline-end placement, use an existing compatible track, or create a new top video/audio track. Defaults to previewOnly; applying is undoable in CDX Studio.',
      inputSchema: {
        type: 'object',
        properties: {
          assetId: {
    ...
    },
    {
      ...
      description: 'Preview or replace an existing video/image/audio timeline clip with another project asset while preserving its timeline slot, transform, label color, effects, and keyframes by default. The old asset is not deleted. Defaults to previewOnly and is undoable in CDX Studio.',
      inputSchema: {
        type: 'object',
        properties: {
          clipId: { type: 'string', description: 'Timeline clip ID to replace. If omitted, CDX Studio must have exactly one selected clip.' },
    ...
    },
    {
      ...
      description: 'Preview or create a CDX Studio adjustment clip on a video track. Adjustment clips apply color/blur/GLSL/keyframed effects to clips below them. Use previewOnly first; applying is undoable in CDX Studio.',
      inputSchema: {
        type: 'object',
        properties: {
    ...
    },
    {
      ...
      description: 'Preview or place multiple project assets onto the active timeline as review lanes or a sequential run. Can target explicit asset IDs/names or the latest matching generated assets, create one new top track per asset, or place sequentially on one compatible track. Defaults to previewOnly; applying is undoable in CDX Studio.',
      inputSchema: {
        type: 'object',
        properties: {
          assets: {
    ...
    {
      name: 'duplicate_clip',
      description: 'Duplicate an existing timeline clip onto the same or another compatible track, preserving clip style, transform, effects, and keyframes. Useful for layered text/title effects. Undoable in CDX Studio; use previewOnly first.',
      inputSchema: {
        type: 'object',
        properties: {
          clipId: {
    ...
    {
      name: 'add_text_clip',
      description: 'Create a CDX Studio text clip on a video track, optionally with typography, transform, timing, preset animation, or explicit transform/text-color keyframes. Undoable in CDX Studio.',
      inputSchema: {
        type: 'object',
        properties: {
          ...
          animationPreset: { type: 'string', enum: ['fade', 'slideUp', 'slideDown', 'slideLeft', 'pop', 'spinIn', 'none'], description: 'Optional existing CDX Studio text animation preset.' },
    ...
    {
      name: 'add_shape_clip',
      description: 'Create a CDX Studio shape clip on a video track for motion graphics: rectangle, rounded rectangle, ellipse, polygon, or line. Use polygon with sides=3 for triangles, 6 for hexagons, 8 for octagons, etc. Supports solid, linear-gradient, and radial-gradient fills, stroke styling, transform, crop, blur, 2.5D rotation, and explicit visual keyframes including width, height, fillOpacity, gradient angle/center/radius, strokeWidth, strokeOpacity, cornerRadius, and polygon sides. Undoable in CDX Studio.',
      inputSchema: {
        type: 'object',
        properties: {
    ...
    },
    {
      ...
      description: 'List the GLSL/GPU-backed clip effects CDX Studio can add through MCP, including supported parameter keys, ranges, defaults, and presets. Use this before adding or changing GLSL effects.',
      inputSchema: {
        type: 'object',
        properties: {
    ...
    },
    {
      ...
      description: 'Add a GLSL/GPU-backed effect to a visual clip. Supports presets, static parameter settings, and optional effect-parameter keyframes. Defaults to previewOnly; applying is undoable in CDX Studio.',
      inputSchema: {
        type: 'object',
        properties: {
    ...
    },
    {
      ...
      description: 'Start a CDX Studio timeline export using the existing hidden export worker. Defaults to MP4 H.264 1080p HD with AAC audio, written to the project renders folder. Use check_export_readiness first for final delivery.',
      inputSchema: {
        type: 'object',
        properties: {
          target: {
          ...
          useHardwareEncoder: {
            type: 'boolean',
            description: 'Request platform hardware encoding using the active export FFmpeg. CDX Studio falls back safely to software encoding if the encoder cannot initialize. Defaults to false.',
          },
          useProxyMedia: {
            type: 'boolean',
    ...
    {
      name: 'export_fcpxml',
      description: 'Preview or export the active CDX Studio timeline as modern FCPXML for Resolve/Final Cut or legacy XMEML v5 for Adobe Premiere Pro. Writes to the project renders folder unless outputPath is provided.',
      inputSchema: {
        type: 'object',
        properties: {
          format: {
    ...
    },
    {
      ...
      inputSchema: {
        ...
        properties: {
          ...
          useHardwareEncoder: { type: 'boolean', description: 'Shared hardware encoder request. CDX Studio probes the active export FFmpeg and falls back to software if unavailable.' },
...
class ComfyStudioMcpServer {
  ...
  async callTool(name, args = {}) {
    ...
    if (!hasSnapshot(snapshot) && !toolsAllowedWithoutProject.has(name)) {
      return errorResult('No CDX Studio project is open yet.')
    }
  ...
  async runComfyUIConnectionDiagnosis(args = {}) {
    if (!this.diagnoseComfyUIConnection) {
      return errorResult('ComfyUI connection diagnostics are not available. Restart CDX Studio and try again.')
    }
    try {
  ...
  async setComfyUIConnectionTool(args = {}) {
    if (!this.setComfyUIConnection) {
      return errorResult('ComfyUI connection setting is not available. Restart CDX Studio and try again.')
    }
  ...
  async repairComfyUIConnection(args = {}) {
    if (!this.diagnoseComfyUIConnection || !this.setComfyUIConnection) {
      return errorResult('ComfyUI connection repair is not available. Restart CDX Studio and try again.')
    }
    ...
    if (!reachable) {
      return textResult({
        ...
        needed: true,
        repaired: false,
        summary: 'CDX Studio is pointed at an unreachable ComfyUI port, and no reachable ComfyUI server was found on the probed local ports.',
        configured,
        probes,
        recommendations: [
    ...
    return textResult({
      ...
      summary: previewOnly
        ? `CDX Studio is pointed at ${configured?.connection?.httpBase}, but ComfyUI is reachable at ${reachable?.connection?.httpBase}. This repair would switch CDX Studio to port ${reachablePort}.`
        : `CDX Studio was switched to the reachable ComfyUI port ${reachablePort}.`,
      configured,
      reachable,
      probes,
      proposedChange: {
  ...
  getComfySetupInstallSteps(installType = 'unknown', diagnosis = null) {
    ...
    const commonFirstStep = `CDX Studio connects to ComfyUI by local URL, usually http://127.0.0.1:${configuredPort}. The important thing is that the port ComfyUI prints must match Settings > ComfyUI Connection in CDX Studio.`
    ...
    if (mode === 'portable') {
      return [
        ...
        'Use that port in CDX Studio Settings > ComfyUI Connection, or ask the MCP agent to apply the suggested port fix.',
        'If CDX Studio launches ComfyUI for you, configure Settings > ComfyUI Launcher to point at that same .bat file.',
      ]
    }
    ...
    if (mode === 'desktop') {
      return [
        ...
        'Set the same port in CDX Studio Settings > ComfyUI Connection.',
    ...
    if (mode === 'docker') {
      return [
        ...
        'From Windows/macOS, CDX Studio should still connect to localhost/127.0.0.1 on the published host port, not the container internal IP.',
        'If ComfyUI returns HTTP 403, relaunch it with a permissive CORS/header setup or use the CDX Studio launcher path when possible.',
      ]
    }
    ...
    if (mode === 'manual') {
      return [
        commonFirstStep,
        'Start ComfyUI manually from your terminal before opening or using CDX Studio generation features.',
        'Look for the URL ComfyUI prints, usually http://127.0.0.1:8188.',
        'If you deliberately use another port, set that exact port in CDX Studio Settings > ComfyUI Connection.',
        'If CDX Studio can see /system_stats but not /object_info, update ComfyUI and check custom-node import errors in the ComfyUI terminal.',
      ]
    }
    ...
    return [
      ...
      'If CDX Studio is on the wrong port, the MCP guide can propose the exact port change and apply it only after approval.',
  ...
  buildComfySetupUserMessage({ configured, reachable, probes, installType, previewOnly, appliedFix, validation }) {
    ...
    if (configured?.connection?.ok) {
      ...
      return [
        `Good news: CDX Studio is already connected to ComfyUI at ${configuredBase}.`,
        ...
        'You can now use local ComfyUI workflows from CDX Studio.',
      ].filter(Boolean)
    }

    if (reachable) {
      return [
        `I found ComfyUI running at ${reachableBase}, but CDX Studio is currently pointed at ${configuredBase}.`,
        previewOnly
          ? `No settings were changed. The safe fix is to set CDX Studio's ComfyUI port to ${reachable.connection.port}.`
          : appliedFix
            ? `I updated CDX Studio to use ${reachableBase}.`
  ...
  async guideComfyUISetup(args = {}) {
    if (!this.diagnoseComfyUIConnection) {
      return errorResult('ComfyUI setup guidance is not available. Restart CDX Studio and try again.')
    }
    ...
    const summary = connected
      ? 'CDX Studio is connected to ComfyUI.'
      : reachable
        ? 'ComfyUI is running, but CDX Studio needs a port-setting fix.'
  ...
  async controlComfyUILauncherTool(args = {}) {
    if (!this.controlComfyLauncher) {
      return errorResult('ComfyUI launcher control is not available. Restart CDX Studio and try again.')
    }
  ...
  async getComfyUILauncherLogsTool(args = {}) {
    if (!this.getComfyLauncherLogs) {
      return errorResult('ComfyUI launcher logs are not available. Restart CDX Studio and try again.')
    }
    try {
  ...
  async runComfyUINodeValidation(args = {}) {
    if (!this.validateComfyUINodes) {
      return errorResult('ComfyUI node validation is not available. Restart CDX Studio and try again.')
    }
    try {
  ...
  async listComfyStudioWorkflowsTool(args = {}) {
    if (!this.listComfyStudioWorkflows) {
      return errorResult('CDX Studio workflow discovery is not available. Restart CDX Studio and try again.')
    }
    try {
      ...
      if (result?.success === false) return errorResult(result.error || 'Could not list CDX Studio workflows.')
      return textResult(result)
    } catch (error) {
      return errorResult(`Could not list CDX Studio workflows: ${error?.message || String(error)}`)
    }
  }

  async inspectComfyStudioWorkflowTool(args = {}) {
    if (!this.inspectComfyStudioWorkflow) {
      return errorResult('CDX Studio workflow inspection is not available. Restart CDX Studio and try again.')
    }
    try {
      ...
      if (result?.success === false) return errorResult(result.error || 'Could not inspect CDX Studio workflow.')
      return textResult(result)
    } catch (error) {
      return errorResult(`Could not inspect CDX Studio workflow: ${error?.message || String(error)}`)
    }
  }
  ...
  async runRendererActionTool(action, args = {}, options = {}) {
    ...
    if (!this.performAction) {
      return errorResult(`${bridgeName} is not available. Restart CDX Studio and try again.`)
    }
    const payload = { ...(args || {}) }
    ...
    try {
      ...
      return textResult({
        ...
        message: result?.message || (previewOnly
          ? `${suggestedTool} plan returned by CDX Studio.`
          : `${suggestedTool} applied through CDX Studio.`),
        result,
  ...
  async inspectMusicVideoKeyframeTool(args = {}) {
    if (!this.performAction) {
      return errorResult('MCP Music Video keyframe inspection is not available. Restart CDX Studio and try again.')
    }
    try {
  ...
  async inspectMusicVideoVideoTool(snapshot, args = {}) {
    if (!this.performAction) {
      return errorResult('MCP Music Video Step 5 inspection is not available. Restart CDX Studio and try again.')
    }
    try {
      const result = await this.performAction({
  ...
  async importComfyUiWorkflowTool(args = {}) {
    if (!this.performAction) {
      return errorResult('MCP workflow import bridge is not available. Restart CDX Studio and try again.')
    }
    ...
    try {
      ...
      return textResult({
        ...
        message: result?.message || (previewOnly
          ...
          : 'Workflow imported into CDX Studio.'),
        result,
        suggestedApplyCall: previewOnly ? {
  ...
  async runMcpActionPlan(snapshot, args = {}) {
    ...
    if (!this.performAction) {
      return errorResult('MCP action-plan bridge is not available. Restart CDX Studio and try again.')
    }
  ...
  async prepareGenerationFromTimelineContext(snapshot, args = {}) {
    ...
    if (!this.performAction) {
      return errorResult('MCP Generate preparation bridge is not available. Restart CDX Studio and try again.')
    }
    ...
    return textResult({
      success: true,
      action: 'prepare_generation_from_timeline_context',
      message: 'Timeline frame captured and sent to Generate. Review settings in CDX Studio, then click Generate manually when ready.',
      plan: {
        ...plan,
        previewOnly: false,
      },
  ...
  async queuePreparedGeneration(_snapshot, args = {}) {
    if (!this.performAction) {
      return errorResult('MCP Generate queue bridge is not available. Restart CDX Studio and try again.')
    }
    ...
    return textResult({
      ...
      message: previewOnly
        ...
        : 'Prepared Generate request queued through CDX Studio.',
      result,
    })
  }
  ...
  async queueTimelineGenerationBatch(snapshot, args = {}) {
    ...
    if (!this.performAction) {
      return errorResult('MCP timeline generation batch bridge is not available. Restart CDX Studio and try again.')
    }
    ...
    return textResult({
      ...
      previewOnly: false,
      action: 'queue_timeline_generation_batch',
      message: `Queued ${plan.totalJobs} timeline generation job${plan.totalJobs === 1 ? '' : 's'} through CDX Studio.`,
      plan,
      result,
    })
  }
  ...
  async queueTimelineTemplateGeneration(snapshot, args = {}) {
    ...
    if (!this.performAction) {
      return errorResult('MCP template generation bridge is not available. Restart CDX Studio and try again.')
    }
    ...
    return textResult({
      ...
      previewOnly: false,
      action: 'queue_timeline_template_generation',
      message: `Queued "${plan.template?.title || plan.template?.name || 'ComfyUI template'}" from the timeline source clip through CDX Studio.`,
      plan,
      result,
    })
  }
  ...
  async queueH3ReferenceVideo(snapshot, args = {}) {
    ...
    if (!imageAsset) return errorResult(`Unknown CDX Studio image asset ID: ${imageAssetId}`)
    if (!audioAsset) return errorResult(`Unknown CDX Studio audio asset ID: ${audioAssetId}`)
  ...
  async resolvePromptBatchMyWorkflows(args = {}) {
    ...
    if (!this.listComfyStudioWorkflows) {
      return { error: 'My Workflows discovery is unavailable. Restart CDX Studio and try again.' }
    }
    ...
    for (const workflowId of requestedIds) {
      ...
      if (entry.mcpRunnable !== true) {
        return {
          error: `Saved workflow "${entry.label || workflowId}" is visible but not agent-ready. ${entry.readinessMessage || 'Add the required CDX Studio marker nodes and save it again.'}`,
        }
      }
  ...
  async queuePromptGenerationBatch(snapshot, args = {}) {
    ...
    if (!this.performAction) {
      return errorResult('MCP prompt generation batch bridge is not available. Restart CDX Studio and try again.')
    }
    ...
    return textResult({
      ...
      previewOnly: false,
      action: 'queue_prompt_generation_batch',
      message: `Queued ${plan.totalJobs} prompt generation job${plan.totalJobs === 1 ? '' : 's'} through CDX Studio.`,
      plan,
      result,
    })
  }
  ...
  async inspectTimelineFrame(snapshot, args = {}) {
    ...
    if (includeImage) {
      if (!this.performAction) {
        warning = 'MCP frame capture bridge is not available. Restart CDX Studio and try again.'
      } else {
        try {
  ...
  async setClipLabelColor(snapshot, args = {}) {
    ...
    if (!this.performAction) {
      return errorResult('MCP write bridge is not available. Restart CDX Studio and try again.')
    }
  ...
  async createProject(snapshot, args = {}) {
    if (!this.performAction) {
      return errorResult('MCP project bridge is not available. Restart CDX Studio and try again.')
    }
  ...
  async addTrack(snapshot, args = {}) {
    ...
    if (!this.performAction) {
      return errorResult('MCP track bridge is not available. Restart CDX Studio and try again.')
    }
  ...
  async updateTrack(snapshot, args = {}) {
    ...
    if (!this.performAction) return errorResult('MCP track bridge is not available. Restart CDX Studio and try again.')
    const result = await this.performAction({
      action: 'update_track',
  ...
  async switchTimeline(snapshot, args = {}) {
    ...
    if (!this.performAction) return errorResult('MCP timeline bridge is not available. Restart CDX Studio and try again.')
    const result = await this.performAction({
      action: 'switch_timeline',
  ...
  async addTransition(snapshot, args = {}) {
    ...
    if (!this.performAction) return errorResult('MCP transition bridge is not available. Restart CDX Studio and try again.')
    const result = await this.performAction({
      action: 'add_transition',
  ...
  async moveClips(snapshot, args = {}) {
    ...
    if (!this.performAction) return errorResult('MCP clip edit bridge is not available. Restart CDX Studio and try again.')
    const result = await this.performAction({
      action: 'move_clips',
      payload: { ...args, previewOnly: false },
    })
  ...
  async createTimeline(snapshot, args = {}) {
    ...
    if (!this.performAction) {
      return errorResult('MCP sequence creation bridge is not available. Restart CDX Studio and try again.')
    }
    ...
    return textResult({
      ...
      message: plan.switchToTimeline
        ? 'Sequence created and CDX Studio switched to it.'
        : 'Sequence created.',
      plan,
      result,
    })
  ...
  async createAssetFolder(snapshot, args = {}) {
    ...
    if (!this.performAction) {
      return errorResult('MCP asset folder bridge is not available. Restart CDX Studio and try again.')
    }
  ...
  async moveAssetsToFolder(snapshot, args = {}) {
    ...
    if (!this.performAction) {
      return errorResult('MCP asset move bridge is not available. Restart CDX Studio and try again.')
    }
  ...
  async moveUnusedAssetsToFolder(_snapshot, args = {}) {
    if (!this.performAction) {
      return errorResult('MCP unused asset cleanup bridge is not available. Restart CDX Studio and try again.')
    }
    ...
    try {
      ...
      return textResult({
        ...
        action: 'move_unused_assets_to_folder',
        message: result?.message || (previewOnly
          ? 'Unused asset cleanup plan returned by CDX Studio.'
          : 'Unused assets moved by CDX Studio.'),
        result,
        suggestedApplyCall: previewOnly ? {
  ...
  async addAssetToTimeline(snapshot, args = {}) {
    ...
    if (!this.performAction) {
      return errorResult('MCP asset placement bridge is not available. Restart CDX Studio and try again.')
    }
    ...
    return textResult({
      success: true,
      action: 'add_asset_to_timeline',
      message: 'Asset placed on the timeline through CDX Studio.',
      plan,
      result,
    })
  }
  ...
  async replaceClipWithAsset(_snapshot, args = {}) {
    if (!this.performAction) {
      return errorResult('MCP clip replacement bridge is not available. Restart CDX Studio and try again.')
    }
    ...
    try {
      ...
      return textResult({
        ...
        action: 'replace_clip_with_asset',
        message: result?.message || (previewOnly
          ? 'Clip replacement plan returned by CDX Studio.'
          : 'Clip replaced through CDX Studio.'),
        result,
        suggestedApplyCall: previewOnly ? {
  ...
  async addSolidColor(snapshot, args = {}) {
    ...
    if (!this.performAction) {
      return errorResult('MCP solid color bridge is not available. Restart CDX Studio and try again.')
    }
    ...
    return textResult({
      success: true,
      action: 'add_solid_color',
      message: 'Solid color asset created through CDX Studio.',
      plan,
      result,
    })
  }
  ...
  async addAssetsToTimeline(snapshot, args = {}) {
    ...
    if (!this.performAction) {
      return errorResult('MCP batch asset placement bridge is not available. Restart CDX Studio and try again.')
    }
    ...
    return textResult({
      success: true,
      action: 'add_assets_to_timeline',
      message: 'Assets placed on the timeline through CDX Studio.',
      plan,
      result,
    })
  }
  ...
  async addAdjustmentClip(snapshot, args = {}) {
    ...
    if (!this.performAction) {
      return errorResult('MCP adjustment clip bridge is not available. Restart CDX Studio and try again.')
    }
    ...
    return textResult({
      success: true,
      action: 'add_adjustment_clip',
      message: 'Adjustment clip created through CDX Studio.',
      plan,
      result,
    })
  }
  ...
  async duplicateClip(snapshot, args = {}) {
    ...
    if (!this.performAction) {
      return errorResult('MCP duplicate bridge is not available. Restart CDX Studio and try again.')
    }
  ...
  async addTextClip(snapshot, args = {}) {
    ...
    if (!this.performAction) {
      return errorResult('MCP text bridge is not available. Restart CDX Studio and try again.')
    }
  ...
  async addShapeClip(snapshot, args = {}) {
    ...
    if (!this.performAction) {
      return errorResult('MCP shape bridge is not available. Restart CDX Studio and try again.')
    }
  ...
  async listGlslEffects(args = {}) {
    ...
    return textResult({
      warning: 'Renderer effect registry is not available. Restart CDX Studio for full parameter details.',
  ...
  async addGlslEffect(snapshot, args = {}) {
    ...
    if (!this.performAction) {
      return errorResult('MCP effect bridge is not available. Restart CDX Studio and try again.')
    }
  ...
  async setClipKeyframes(snapshot, args = {}) {
    ...
    if (!this.performAction) {
      return errorResult('MCP clip keyframe bridge is not available. Restart CDX Studio and try again.')
    }
    ...
    return textResult({
      success: true,
      action: 'set_clip_keyframes',
      message: 'Clip keyframes updated through CDX Studio.',
      plan,
      result,
    })
  }
  ...
  async addDipToBlack(_snapshot, args = {}) {
    if (!this.performAction) {
      return errorResult('MCP dip-to-black bridge is not available. Restart CDX Studio and try again.')
    }
    ...
    try {
      ...
      return textResult({
        ...
        action: 'add_dip_to_black',
        message: result?.message || (previewOnly
          ? 'Dip-to-black opacity keyframe plan returned by CDX Studio.'
          : 'Dip-to-black opacity keyframes applied through CDX Studio.'),
        result,
        suggestedApplyCall: previewOnly ? {
  ...
  async exportTimeline(snapshot, args = {}) {
    ...
    if (!this.performAction) {
      return errorResult('MCP export bridge is not available. Restart CDX Studio and try again.')
    }
    ...
    return textResult({
      success: true,
      action: 'export_timeline',
      message: 'Export started in the CDX Studio export worker.',
      ...exportPlan,
      result,
    })
  }
  ...
  async exportDeliveryBatch(snapshot, args = {}) {
    ...
    if (!this.performAction) {
      return errorResult('MCP batch export bridge is not available. Restart CDX Studio and try again.')
    }
    ...
    return textResult({
      ...
      action: 'export_delivery_batch',
      message: failedCount === 0
        ? `Started ${results.length} delivery export${results.length === 1 ? '' : 's'} through CDX Studio.`
  ...
  async exportFcpXml(snapshot, args = {}) {
    ...
    const filename = String(args.filename || `${snapshot.project?.name || 'CDX Studio'}_${timeline.name || 'Timeline'}`).trim()
    const outputPath = String(args.outputPath || '').trim()
    const plan = {
      projectPath,
      outputPath: outputPath || 'project renders folder',
      filename: filename || `${snapshot.project?.name || 'CDX Studio'}_${timeline.name || 'Timeline'}`,
    ...
    if (!this.performAction) {
      return errorResult(`MCP ${formatLabel} export bridge is not available. Restart CDX Studio and try again.`)
    }
    ...
    return textResult({
      success: true,
      action: 'export_fcpxml',
      message: `${formatLabel} exported through CDX Studio.`,
      plan,
      result,
    })
  }

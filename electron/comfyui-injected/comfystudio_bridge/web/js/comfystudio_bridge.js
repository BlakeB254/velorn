...
function createBridgeButton() {
  ...
  button.type = "button";
  button.textContent = "Send to CDX Studio";
  button.title = "Export this graph as API JSON and send it back to CDX Studio.";
  ...
  button.addEventListener("click", async () => {
    ...
    try {
      ...
      setStatus(sent ? "Sent to CDX Studio." : "Open ComfyUI inside CDX Studio to send directly.", sent ? "ok" : "error");
      window.setTimeout(() => setStatus("", "idle"), sent ? 2500 : 6500);
    } catch (error) {
      setStatus(error?.message || "Could not send workflow to CDX Studio.", "error");
    } finally {
